...
function Sidebar({
  ...
  return (
    <div
      className="group peer text-sidebar-foreground hidden md:block border-r border-l border-[#f4f1fa] dark:border-[#212024]"
      data-state={state}
...
function SidebarInset({ className, ...props }: React.ComponentProps<'main'>) {
  return (
    <main
      ...
      className={cn(
        'bg-background relative flex w-full flex-1 flex-col m-0!',
        'h-[calc(100vh)] md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow-sm md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2',
        className
      )}
      {...props}
    />
...
// ## The Manager patterns starts from here ##
...
function SidebarManagerTrigger({
  ...
  const handleClickSidebarTrigger = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const screenWidth = window.innerWidth;
    const totalWith =
      Number(SIDEBAR_WIDTH.replace('rem', '')) * sidebarList.length * rootFontSize +
      Number(MIN_CONTENT_WIDTH.replace('px', ''));

    if (totalWith > screenWidth && !sidebar?.open) {
      sidebarList.forEach((sidebarName) => {
        if (sidebarName !== name) {
          manager.use(sidebarName)?.setOpen(false);
        }
      });
    }
