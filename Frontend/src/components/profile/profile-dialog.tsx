...
export const MyProfileDialog = ({ open, onOpenChange }: ProfileDialogProps) => {
  ...
  const [mode, setMode] = useState<'view' | 'edit-info' | 'edit-avatar' | 'edit-bg'>('view');
    ...
    <Dialog open={open} onOpenChange={onOpenHandler}>
      <DialogContent aria-describedby={undefined}>
        ...
        <div className="w-full overflow-hidden">
          ...
          >
            <div className={cn('flex-1 max-w-1/2', mode !== 'view' ? 'h-0' : 'h-auto')}>
              {user && (
                <ProfileCard
                  user={user}
                  onBgClick={() => {
              ...
              <Button variant="primary" className="w-full" onClick={() => setMode('edit-info')}>
                <PenLine />
                Chỉnh sửa thông tin
              </Button>
            </div>
